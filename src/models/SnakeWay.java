...
import List_Boss.SnakeWaySAIBAMEN;
import List_Boss.SnakeWayNADIC;
import List_Boss.SnakeWayCADICH;
import boss.Boss;
    ...
    private void init() {
        ...
        for (Zone zone : this.zones) {
            ...
            for (int i = 0; i < mobs.size(); i++) {
                ...
                if (i == 5) {
                    mob.lvMob = 1;
                    mob.point.dame = (level * 100 * mob.tempId * 12);
                    mob.point.maxHp = (level * 1000 * mob.tempId * 12);
                    mob.hoiSinh();
                    mob.hoiSinhMobPhoBan();
                } else {
                    mob.lvMob = 0;
                    mob.point.dame = (level * 100 * mob.tempId);
                    mob.point.maxHp = (level * 1000 * mob.tempId);
                    mob.hoiSinh();
                    mob.hoiSinhMobPhoBan();
                }
            }
            ...
            if (zone.map.mapId == 144) {
                try {
                    ...
                    for (int i = 6; i > 0; i--) {
                        ...
                        bosses.add(new SnakeWaySAIBAMEN(
                                zone,
                                clan,
                                i,
                                bossDamage,
                    ...
                    bossMaxHealth = ((bossMaxHealth * 5));
                    bosses.add(new SnakeWayNADIC(
                            zone,
                            clan,
                            bossDamage,
                            bossMaxHealth
                    ...
                    bossMaxHealth = ((bossMaxHealth * 10));
                    bosses.add(new SnakeWayCADICH(
                            zone,
                            clan,
                            bossDamage,
                            bossMaxHealth
