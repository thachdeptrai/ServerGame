...
public class Mob {
    ...
    public void injured(Player plAtt, double damage, boolean dieWhenHpFull) {
        if (!this.isDie()) {
            ...
            if (plAtt != null) {
                ...
                Service.gI().addSMTN(plAtt, (byte) 2, getTiemNangForPlayer(plAtt, (long) damage), true);
                TrainingService.gI().tangTnsmLuyenTap(plAtt, getTiemNangForPlayer(plAtt, (long) damage));
            }
        }
    }
    ...
    public long getTiemNangForPlayer(Player pl, long dame) {
        ...
        if (pl.itemTime != null && pl.itemTime.isUseCUUCHUYENKIMDAN) {
//            PlayerService.gI().sendTNSM(player, (byte)2, 100_000_000_000L);
            tiemNang *= 5;
        }
        return tiemNang;
    }
...
//            }
        ...
        // Vang roi
        if (Util.isTrue(10, 100) || (Manager.TEST && Util.isTrue(1, 10)) || (player.actived() && Util.isTrue(1, 10))) {
        ...
        //Set kich hoat
        if (((Util.isTrue(3, 100)) || (Manager.TEST && Util.isTrue(4, 100)) || Util.isTrue(5, 100)) && MapService.gI().isMapUpSKH(mapid)) {
...
//            }
            ...
            if (Util.isTrue(1, 400) && InventoryService.gI().fullSetThan(player)) {
             ...
             if (Util.isTrue(1, 400) && InventoryService.gI().fullSetThienSu(player)) {
                ItemMap it = new ItemMap(zone, Util.nextInt(1763, 1767), 1, x, yEnd, player.id);
                it.options.add(new Item.ItemOption(30, 0));
                list.add(it);
            }
        }
        ...
        if ((Util.isTrue(1, 1000)&& MapService.gI().isMapTuTien(mapid) || (player.actived() && Util.isTrue(1, 300))) && MapService.gI().isMapTuTien(mapid) || (player.isAdmin() && Util.isTrue(1, 2))) {
            list.add(new ItemMap(zone, Util.nextInt(1754, 1756), 1, x, yEnd, player.id));
        }
        // Manh thien su
