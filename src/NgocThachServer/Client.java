...
public class Client implements Runnable {
    ...
    private void remove(MySession session) {
        ...
        if (session.joinedGame) {
            ...
            try {
                NNTDB.executeUpdate("update account set last_time_logout = ? where id = ?", new Timestamp(System.currentTimeMillis()), session.userId);
            } catch (Exception e) {
                Logger.logException(Client.class, e);
            }
        }
        NgocRongOnline.gI().disconnect(session);
    }
