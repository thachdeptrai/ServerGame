...
import services.SuperRankService;
...
import models.Archivement;
import player.ArchivementSucManh;
import services.InventoryService;
import services.PetService;
import FunC.LuckyRound;
import FunC.TransactionService;
import skill.Skill;
import utils.Logger;
import DBConnect.NTTResultSet;
import FunC.VongQuayMayManService;
import java.io.File;
import services.VQMMService;
        ...
        try {
            ...
            switch (cmd) {
                ...
                case -99:
                    ...
                    break;
                case 69:
                    VongQuayMayManService.gI().readData(_msg, player);
                    break;
                case 70:
//                    VQMMService.readData(_msg, player);
                    break;
                case 18:
                    if (player != null) {
...
//                    if (SessionZ.NNTServer.firewallDownDataGame.containsKey(ip)) {
//                        int soLanConnect = SessionZ.NNTServer.firewallDownDataGame.get(ip).intValue();
//                        if (soLanConnect > 21) {
//                            Service.gI().sendThongBaoOK(_session, "Bạn đã tải dữ liệu nhiều lần, đợi bảo trì rồi quay lại");
//                            return;
//                        } else {
//                            SessionZ.NNTServer.firewallDownDataGame.put(ip, soLanConnect += 1);
//                        }
//
//                    } else {
//
//                        SessionZ.NNTServer.firewallDownDataGame.put(ip, 1);
//                    }
...
//                    break;
                case -74:
                    ...
                    String ip = _session.ipAddress;
                    if (SessionZ.NNTServer.firewallDownDataGame.containsKey(ip)) {
                        int soLanConnect = SessionZ.NNTServer.firewallDownDataGame.get(ip);
                        if (soLanConnect > 21) {
                            Service.gI().sendThongBaoOK(_session, "Bạn đã tải dữ liệu nhiều lần, đợi bảo trì rồi quay lại");
                            return;
                        } else {
                    ...
                    try {
                        byte type = _msg.reader().readByte();
                        long afterReadByte = System.currentTimeMillis();
                        if (type == 1) {
                            DataGame.sendSizeRes(_session);
                        } else if (type == 2) {
                            DataGame.sendRes(_session);
        ...
        if (msg != null) {
            try {
                if (msg.reader().available() < 1) {
                    System.out.println("Dữ liệu không đủ để đọc cmd.");
                    return;
                }
                byte cmd = msg.reader().readByte();
                ...
                switch (cmd) {
                    case 0:
                        if (msg.reader().available() >= 4) {  // Kiểm tra dữ liệu trước khi đọc
                ...
                System.out.println("Lỗi khi xử lý messageNotLogin: " + e.getMessage());
                session.disconnect();
                Logger.logException(Controller.class, e);
            }
        }
                    ...
                    case 13:
                        ...
                        if (player != null && player.isPl()) {
                            ...
                            if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
                                ...
                                String npcSay = "Chào mừng " + player.name + "Đã trở lại với Ngọc Rồng " + NgocRongOnline.NAME + "\n"
                                        + "Mọi chi tiết về Game vui lòng bấm vào mục Chức Năng -> Thông báo để tham khảo";
                                NpcService.gI().createTutorial(player, -1, npcSay);
                            }
        ...
        if (!Maintenance.isRunning) {
            ...
            try {
                ...
                if (name.length() >= 5 && name.length() <= 20) {
                    rs = NNTDB.executeQuery("select * from player where name = ?", name);
                    if (rs.next()) {
...
//                        }
                    ...
                    Service.gI().sendThongBaoOK(session, "Tên nhân vật chỉ đồng ý các ký tự a-z, 0-9 và chiều dài từ 5 đến 20 ký tự");
                }
            } catch (Exception e) {
                Logger.logException(Controller.class, e);
            } finally {
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "|5|Chào Mừng\n"
                + player.name + "\n"
                + "Đã trở lại với Ngọc Rồng " + NgocRongOnline.NAME
                + "\n|6|Nếu gặp bất kì lỗi nào trong quá trình trải nghiệm game \n"
                + "|6|Hoặc có ý tưởng nào muốn trải nghiệm thử\n"
                + "|6|Vui lòng đăng bài lên diễn đàn của game\n"
                + "|6|Chúc các bạn chơi game vui vẻ");
    }
