...
import List_Boss.TrainingToSuKaio;
import List_Boss.TrainingYajiro;
import List_Boss.TrainingMrPoPo;
import List_Boss.TrainingKhiBubbles;
import List_Boss.TrainingThuongDe;
import List_Boss.TrainingWhis;
import List_Boss.TrainingTauPayPay;
import List_Boss.TrainingKarin;
import List_Boss.TrainingThanVuTru;
import boss.Boss;
import boss.BossID;
...
public class TrainingService {
    ...
    public Boss callBoss(Player pl, int bossID, boolean isThachDau) {
        try {
            ...
            switch (bossID) {
                case BossID.KARIN -> {
                    return new TrainingKarin(pl);
                }
                case BossID.TAUPAYPAY -> {
                    return new TrainingTauPayPay(pl);
                }
                case BossID.YAJIRO -> {
                    return new TrainingYajiro(pl);
                }
                case BossID.MRPOPO -> {
                    return new TrainingMrPoPo(pl);
                }
                case BossID.THUONG_DE -> {
                    ChangeMapService.gI().changeMap(pl, MapService.gI().getMapCanJoin(pl, 49, 0), 362, 408);
                    return new TrainingThuongDe(pl);
                }
                case BossID.KHI_BUBBLES -> {
                    return new TrainingKhiBubbles(pl);
                }
                case BossID.THAN_VU_TRU -> {
                    return new TrainingThanVuTru(pl);
                }
                case BossID.TO_SU_KAIO -> {
                    return new TrainingToSuKaio(pl);
                }
                case BossID.WHIS -> {
                    return new TrainingWhis(pl);
                }
            }
        } catch (Exception e) {
