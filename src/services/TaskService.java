        ...
        switch (npc.tempId) {
            ...
            case ConstNpc.BARDOCK:
                return (doneTask(player, ConstTask.TASK_31_2)
                        || doneTask(player, ConstTask.TASK_31_4) || doneTask(player, ConstTask.TASK_31_6));
            case ConstNpc.TIENNHAN:
                return (doneTask(player, ConstTask.TASK_32_1)
                        || doneTask(player, ConstTask.TASK_33_1) || doneTask(player, ConstTask.TASK_33_4));
                case ConstNpc.THUONGNHAN:
                    return doneTask(player,ConstTask.TASK_33_3);
        }   
        return false;
    }
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_31_3:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đánh được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                                + transformName(player, " Con"));
                    } else {
                        Service.gI().sendThongBao(player, "Về báo cáo với Ba đóc :))");
                    }
                    break;
                ...
                case ConstTask.TASK_32_1:
                    npcSay(player, ConstNpc.TIENNHAN, "Cháo mừng con đến với vùng đất tiên giới,\n"
                            + "Con muốn bái ta làm sư phụ?,\n"
                            + "Mơ đi,"
                            + "Thôi được rồi ta sẽ cho con 1 nhiệm vụ ,Nếu con có thể hoàn thành ta sẽ nhận con làm đệ tử\n"
                            + "không khác gì hạ giới, bên tiên giới cũng có quái vật\n"
                            + ", Chúng nó tàn phá giấc ngủ của ta mấy hôm nay, con hãy đi tiêu diệt bọn chúng , xong việc ta sẽ xem sét nhận con làm đệ tử");
                    break;
                case ConstTask.TASK_33_0:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đánh được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                                + transformName(player, " Zombie"));
                    } else {
                        Service.gI().sendThongBao(player, "Về báo cáo với Tiên Nhân");
                    }
                    break;
                case ConstTask.TASK_33_1:
                    npcSay(player, ConstNpc.TIENNHAN, "Làm tốt lắm\n ,tối ta có thể ngủ ngon rồi\n"
                            + ",ra kia kiếm cho ta ít linh thạch\n"
                            + " ,mua rượu cho ta");
                    break;
                case ConstTask.TASK_33_2:
                    ...
                    Service.gI().sendThongBao(player, "Bạn thu thập được "
                            + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                            + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount + " Linh thạch cấp 1");
                     }else{
                         Service.gI().sendThongBao(player, "Gặp Thương Nhân Mua Rượu Cho Sư Phụ Thôi Nào");
                     }
                    break;
                case ConstTask.TASK_33_4:
                    npcSay(player, ConstNpc.TIENNHAN, "Búuuuuuuuuuuuuuuuuu");
                    Item i = InventoryService.gI().findItemBag(player, 1757);
                    try {
                        InventoryService.gI().subQuantityItemsBag(player, i, 1);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
    ...
    private void rewardDoneTask(Player player) {
        switch (player.playerTask.taskMain.id) {
            ...
            case 5:
                ...
                break;
            case 32:
                Service.gI().addSMTN(player, (byte) 0, 10_000_000_000L, false);
                Service.gI().addSMTN(player, (byte) 1, 10_000_000_000L, false);
                break;
            case 33:
                Service.gI().addSMTN(player, (byte) 0, 1_000_000_000L, false);
                Service.gI().addSMTN(player, (byte) 1, 1_000_000_000L, false);
                long gold = 1000000000L;
                int itemId = 673;/*telegram @tomihjhj_bot*/
                Item item = ItemService.gI().createNewItem(((short) itemId));
                if (player.inventory.gold < player.inventory.LIMIT_GOLD && player.inventory.gold > 0) {
                    player.inventory.gold += gold;
                    Service.gI().sendThongBao(player, "Bạn Nhận Được: " + Util.numberToMoney(gold) + " Vàng");
                } else {
                    Service.gI().sendThongBao(player, "Vàng đã vượt quá giới hạn bạn nhận được cái nịt");
                    ItemShop it = new Shop().getItemShop(itemId);/*telegram @tomihjhj_bot*/
                    if (it != null && !it.options.isEmpty()) {
                        item.itemOptions.addAll(it.options);/*telegram @tomihjhj_bot*/
                    }
