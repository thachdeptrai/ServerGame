...
//                break;                
    ...
    public boolean x99ThucAnTien(Player player) {
        Item doAnTien = player.inventory.itemsBag.stream().filter(it -> it != null && it.template != null && (it.template.id == 1763 || it.template.id == 1764 || it.template.id == 1765 || it.template.id == 1766 || it.template.id == 1767) && it.quantity >= 99).findFirst().orElse(null);
        return doAnTien != null;
    }
