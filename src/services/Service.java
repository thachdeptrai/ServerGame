...
public class Service {
    ...
    public String checkInfomationGiftCode() {
        ...
        try {
            NTTResultSet rs = NNTDB.executeQuery("SELECT * FROM Giftcode WHERE count_left > 0");
            while (rs.next()) {
            ...
            System.out.println("loi checkInfomationGiftCode ");
        }
        return textGift;
    }
...
//        }
    ...
    public void regisAccount(Session session, Message _msg) {
        try {
            ...
            NTTResultSet rs = NNTDB.executeQuery("select * from account where username = ?", user);
            if (rs.next()) {
                sendThongBaoOK((MySession) session, "Tài khoản đã tồn tại");
            } else {
                NNTDB.executeUpdate("insert into account (username, password, admin , cash, danap) values()", user, pass, 0, 0, 0);
...
//         
        if (player.isPet) {
            if (player.nPoint.power > player.nPoint.getPowerLimit()) {
                ...
                player.nPoint.power += param;/*telegram @tomihjhj_bot*/
...
//    }
    ...
    public void charDie(Player pl) {
        ...
        Message msg;
        try {
            if (!pl.isPet && !pl.isNewPet && pl.isPl() && !pl.isBot) {
                msg = new Message(-17);
            ...
            if (newPass.length() >= 6) {
                if (newPass.equals(rePass)) {
                    ...
                    try {
                        ...
                        NNTDB.executeUpdate("update account set password = ? where id = ? and username = ?",
...
//                if (key.equals("ngthach") && code == 31072002 && token.equals("ngocthach")) {
...
//            }
    ...
    public void callNhanBan(Player player) {
        ...
        BossData bossDataClone = new BossData(
                ...
                new double[]{((player.nPoint.hpMax) * 10L)},
                new int[]{140},
                skillTemp,
