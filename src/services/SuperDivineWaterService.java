 ...
 * @author NgocThach
 */
import List_Boss.PocoloSuperDivineWater;
...
public class SuperDivineWaterService {
    ...
    public void init(Zone zone, Player player) {
        ...
        for (Mob mob : zone.mobs) {
            switch (mob.tempId) {
                case 26:
                    mob.point.dame = ((totalHp * 150 / 100));
                    mob.point.maxHp = ((totalDamage * 150 / 100));
                    break;
                case 25:
                    mob.point.dame = ((totalHp * 2));
                    mob.point.maxHp = ((totalDamage * 2));
                    break;
                default:
                    mob.point.dame = ((totalHp));
                    mob.point.maxHp = ((totalDamage));
                    break;
            }
            mob.lvMob = 0;
            mob.hoiSinh();
            ...
            if (player.isPl() && MapService.gI().isMapSieuThanhThuy(player.zone.map.mapId) && player.zone == player.zoneSieuThanhThuy) {
                if (!player.callBossPocolo) {
                    ...
                    if (allCharactersDead) {
                        try {
                            double bossDamage =  (player.nPoint.dame);
                            double bossMaxHealth = ( player.nPoint.hpMax * 5);
                            new PocoloSuperDivineWater(
                                    player.zone,
                                    player,
                                    bossDamage,
                                     bossMaxHealth
