...
public class NangCapLinhThach {

    public static void showInfoCombineNLT(Player player) {
        ...
        if (player.combine.itemsCombine.size() != 1) {
            Service.gI().sendDialogMessage(player, "Cần 100 viên Linh Thạch");
            return;
        }
        Item item = player.combine.itemsCombine.get(0);
        if (item == null || !item.isNotNullItem() || item.template.id < 1750
                && item.template.id > 1756 || item.quantity < 100) {
            Service.gI().sendDialogMessage(player, "Cần 100 viên Linh thạch");
            System.out.println("sai đk");
            return;
        }
        if (item.template.id == 1750 || item.template.id == 1751 || item.template.id == 1752
                || item.template.id == 1753 || item.template.id == 1754 || item.template.id == 1755
                || item.template.id == 1756) {
            StringBuilder text = new StringBuilder();
            text.append(ConstFont.BOLD_BLUE).append("Con có muốn biến 100 ").append(item.template.name).append(" thành\n");
            text.append("1 viên ").append(ItemService.gI().getTemplate((short) (item.template.id + 1)).name).append("\n");
            text.append(ConstFont.BOLD_GREEN).append("Cần 100 ").append(item.template.name);
...
//        }
    ...
    public static void nhapLinhThach(Player player) {
        if (InventoryService.gI().getCountEmptyBag(player) == 0) {
            return;
        }
        if (player.combine.itemsCombine.size() != 1) {
        ...
        Item item = player.combine.itemsCombine.get(0);
        if (item == null || !item.isNotNullItem() || item.template.id < 1750 || item.template.id > 1756 || item.quantity < 100) {
            System.out.println("sai điều kiện");
            return;
        }
        if (item.template.id > 1750 && item.template.id < 1756 ) {
            Item ngocRong = InventoryService.gI().findItemBag(player, (short) (item.template.id + 1));
            if (ngocRong != null && ngocRong.quantity >= 1000) {
