...
import models.Traning;
import EventZ.ItemEvent;
import data.BadgesData;
import skill.EffectSkill;
import skill.NewSkill;
...
import services.MajinBuuService;
...
import MapZ.Zone;
import Interface.IPVP;
import models.TYPE_LOSE_PVP;
import skill.Skill;
...
import MapZ.ItemMap;
import MapZ.MaBuHold;
import models.TranhNgoc;
import services.TranhNgocService;
import models.MajinBuu14H;
import services.SuperDivineWaterService;
import models.The23rdMartialArtCongress;
import EventZ.ShenronEvent;
...
public class Player implements Runnable {
    ...
    public int CheckDayOnl;
    public byte diemdanh;
    public int Month_Card;
    public long timevip;
    public int vip;
    public boolean giveWaterBottle;
...
//                        }
    ...
    public void updateCSMM() {
        MiniGameZ.LuckyNumber.players.forEach((g) -> {
            if (this.id == g.id) {
...
//            damage = Math.min(damage, 2_000_000_000L); // Giới hạn damage không vượt quá 2 tỷ
    ...
    public void kill(Player pl) {
        pl.injured(this, (pl.nPoint.hpMax), false, false);
...
//    }
    ...
    public String percentGold(int type) {
        try {
            double percent = 0.0; // Mặc định cơ hội trúng là 0%

            if (type == 0) {
                // Kiểm tra xem tổng giải thưởng thường có bằng 0 không để tránh chia cho 0
                if (AllinOne.gI().goldNormar != 0) {
                    percent = ((double) this.goldNormar / AllinOne.gI().goldNormar) * 100;
                }
            } else if (type == 1) {
                // Kiểm tra xem tổng giải thưởng VIP có bằng 0 không để tránh chia cho 0
                if (AllinOne.gI().goldVip != 0) {
                    percent = ((double) this.goldVIP / AllinOne.gI().goldVip) * 100;
                }
            }

            return String.valueOf(Math.ceil(percent)); // Làm tròn giá trị và trả về kết quả
        } catch (Exception e) {
            e.printStackTrace();
...
//        if (Server.S_VIP.contains(name)) {
//            return "[S_VIP]" + name;
//        } else if (Server.Z_VIP.contains(name)) {
//            return "[Z_VIP]" + name;
//        } else if (Server.A_VIP.contains(name)) {
//            return "[A_VIP]" + name;
//        } else if (Server.V_VIP.contains(name)) {
//            return "[V_VIP]" + name;
//        } else {
        return name;
//        }
