...
import MobZ.Mob;
import Manager.PowerLimitManager;
import utils.TimeUtil;

public class Char {

    public static final byte MAX_LIMIT = 13;
    public static final byte MAX_LIMITTuTien = 20;
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseTHANTAMDAN) {
            this.tlNeDon += 10 / 100;
        }
        // Bông tai cấp 2
...
//                break; c
    ...
    private void setHpHoi() {
        ...
        double hpToRestore = this.hpMax * 70 / 100;
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseBOHUYETDAN) {
            this.hpHoi += hpToRestore;
        }
    }
    ...
    private void setMpHoi() {
        ...
        double mpToRestore = this.mpMax * 70 / 100;
    ...
    private void setHpMax() {
        ...
        if (this.player.setClothes.tienNhan == 5) {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseTHIENMENHDAN) {
            hpMax += (hpMax * 15 / 100L);
        }
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseHOIXUANDAN) {
            hpMax *= 10 / 100;
        }
...
//        }
    ...
    private void setMpMax() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseNGUNGTHANDAN) {
            mpMax *= 10 / 100;
        }
        // Phù map mabu
        if (this.player.isPhuHoMapMabu) {
...
//        }
    ...
    private void setDame() {
        ...
        if (this.player.setClothes.sethanhtinh == 5) {
            dame += (dame * 1000 / 100L);
            this.tlDameCrit.add(500);
            this.tlSDCM += 500;
        }
        if (this.player.setClothes.setvutru == 5) {
            dame += (dame * 5000 / 100L);
            this.tlDameCrit.add(2500);
            this.tlSDCM += 2500;
        }
        if (this.player.setClothes.sethoangkim == 5) {
            dame += (dame * 150000 / 100L);
            this.tlDameCrit.add(100000);
            this.tlSDCM += 100000;
        }
        if (this.player.setClothes.settukim == 5) {
            dame += (dame * 1000_000 / 100L);
            this.tlDameCrit.add(50_0000);
            this.tlSDCM += 500_000;
        }
        if (this.player.setClothes.setvongcot == 5) {
            dame += (dame * 2000_000 / 100L);
            this.tlDameCrit.add(1000000);
            this.tlSDCM += 1_000_000;
        }
        if (this.player.setClothes.setanhdao == 5) {
            dame += (dame * 5_000_000 / 100L);
            this.tlDameCrit.add(2_000_000);
            this.tlSDCM += 2_000_000;

        }
        if (this.player.setClothes.sethuyetlong == 5) {
            dame += (dame * 10_000_000 / 100L);
            this.tlDameCrit.add(2_000_000);
            this.tlSDCM += 2_000_000;

        }
        if (this.player.setClothes.setblack == 5) {
            dame += (dame * 15_000_000 / 100L);
            this.tlDameCrit.add(2_000_000);
            this.tlSDCM += 2_000_000;
        }
...
//        }
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseTRUTIENDAN) {
            dame += (dame * 10 / 100L);
        }
        if (this.player.itemTime != null && this.player.itemTime.isUseHOALONGDAN) {
            dame += (dame * 15 / 100L);
        }
        if (this.player.itemTime != null && this.player.itemTime.isUseLoX15) {
            dame -= (dame * 85 / 100L);
...
// Xử lý set nail
        ...
        //đan
        if (this.player.itemTime != null && this.player.itemTime.isUseDIEUAMDAN) {
            this.tlDameCrit.add(20);
        }
        // Xử lý gogeta
        if (this.isGogeta) {
    ...
    public double getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.KAMEJOKO:
                ...
                if (this.player.setClothes.setanhdao == 5) {
                    percentXDame = 5_000_000;
                }
                if (this.player.setClothes.sethuyetlong == 5) {
                    percentXDame = 10_000_000;
                }
                if (this.player.setClothes.setblack == 5) {
                    percentXDame = 15_000_000;
                }
                break;
            case Skill.GALICK:
                if (intrinsic.id == 16) {
            ...
            case Skill.QUA_CAU_KENH_KHI:
                ...
                double dameqckk = (hpmob * 10 / 100) + (hppl * 10 / 100) + this.dame * 10;
...
//                }
        ...
        dameAttack += ((double) dameAttack * percentXDame / 100);

        double tempDameAttack = (double) (dameAttack / 100L * 5L);
        if (tempDameAttack <= 0) {
            tempDameAttack = 1;
        }
        dameAttack += (Util.getOne(-1, 1) * Util.nextDouble((double) tempDameAttack) + 1);
...
//        }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        ...
        if (power < getPowerLimit()) {
            ...
            long tn = tiemNang;
            if (this.player.itemTime != null && this.player.itemTime.isUseLINHHUDAN) {
                tiemNang += tn * 2;
            }
            ...
            if (this.player.itemTime != null && this.player.itemTime.isUseCUUCHUYENKIMDAN) {
                tiemNang += tn * 5;
            }
...
//        tiemNang = (long) (tiemNang * 0.75);
        if (player.nPoint.power >= 800_000_000_000L) {
            tiemNang = tiemNang / 65;
        } else if (player.nPoint.power >= 600_000_000_000L) {
            tiemNang = tiemNang / 50;
        } else if (player.nPoint.power >= 400_000_000_000L) {
            tiemNang = tiemNang / 40;
        }
        if (player.nPoint.power >= 1000_000_000_000L) {
            tiemNang = tiemNang / 70;
        }
        if (player.nPoint.power >= 130_000_000_000_000L) {
            tiemNang = tiemNang / 80;
        }
        if (player.nPoint.power >= 150_000_000_000_000_000L) {
...
//    }
    public long getPowerNextLimit() {
        int nextLimit = limitPower + 1;
        if (nextLimit > 14) {
            return 0; // Trả về 0 nếu vượt giới hạn
        }
        PowerLimit powerLimit = PowerLimitManager.getInstance().get(nextLimit);
        if (powerLimit != null) {
            return powerLimit.getPower();
        }
        return 0;
    ...
    public long getPowerNextLimitTuTien() {
        int nextLimit = limitPower + 1;

        // Chỉ xử lý nếu nextLimit > 14 và <= 20
        if (nextLimit > 14 && nextLimit <= 20) {
            PowerLimit powerLimit = PowerLimitManager.getInstance().get(nextLimit);
            if (powerLimit != null) {
                return powerLimit.getPower();
            }
        ...
        // Trả về 0 nếu không hợp lệ hoặc không tìm thấy
        return 0;
    }
...
//    }
    ...
    public long getFullTN() {
        ...
        if (hpg > 0) {
            tnhp = (long) (((hpg / 20L) * (50L + (50L + (hpg / 20L) - 1L)) / 2L) * 20L);
        }
        if (mpg > 0) {

            tnki = (long) (((mpg / 20L) * (50L + (50L + (mpg / 20L) - 1L)) / 2L) * 20L);
        }
        if (dameg > 0) {
            tnsd = (long) ((dameg * (dameg - 1L) * 100L) / 2L);
        }
        if (defg > 0) {
        ...
        if (critg > 0) {
            ...
            tncm = (long) ((50L * ((Math.pow(5L, critg) - 1L)) / (5L - 1L) * 1000000L));
        }
...
//                    }
                    ...
                    if (player.effectSkill.countCharging % 3 == 0) {
                        Service.gI().chat(player, "Phục hồi năng lượng " + Math.round(getCurrPercentHP()) + "%");
                    }
                } else {
