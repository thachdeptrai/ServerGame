...
import Manager.TopPowerManager;
import Manager.TopTaskManager;
...
//                Manager.topDuaNap = Manager.realTop(ConstSQL.TOP_DUA_NAP, con);
    ...
    public static void openTransactionUI(Player player) {
        TransactionManager.getInstance().load();
        List<Transaction> transactions = TransactionManager.getInstance().getTransactionList();
        transactions.sort((t1, t2) -> t2.getCreatedAt().compareTo(t1.getCreatedAt())); // Sắp xếp theo thời gian giao dịch mới nhất

        Message msg = null;
        try {
            msg = new Message(-96);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("Danh sách giao dịch");
            msg.writer().writeByte(transactions.size());

            for (int i = 0; i < Math.min(100, transactions.size()); i++) {
                Transaction transaction = transactions.get(i);
                msg.writer().writeInt(i + 1);
                msg.writer().writeInt(i + 1);
                msg.writer().writeShort(transaction.getHead());
                if (player.getSession().version >= 214) {
                    msg.writer().writeShort(-1);
                }
                msg.writer().writeShort(transaction.getBody());
                msg.writer().writeShort(transaction.getLeg());
                msg.writer().writeUTF(transaction.name);
                msg.writer().writeUTF("Ngân Hàng: " + transaction.getBankName());  // Ngân hàng
                msg.writer().writeUTF("Số Tiền: " + Util.numberToMoney(transaction.getAmount()) + " VNĐ\n"
                        + "Nội Dung: " + transaction.getDescription() + "\n"
                        + "Trạng Thái Nạp: " + (transaction.isStatusDeposit()
                        ? "Thành Công\n" // Màu vàng
                        : "Chưa Xác Nhận\n") // Màu đỏ
                        + "Trạng Thái Nhận Tiền: " + (transaction.isStatusReceive()
                        ? "Thành Công\n" // Màu vàng
                        : "Chưa Xác Nhận\n") // Màu đỏ
                        + "Thời Gian Giao Dịch: " + transaction.getCreatedAt());
    ...
    public static void showListTopTask(Player player) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                msg.writer().writeShort(top.getLeg());
                msg.writer().writeUTF(top.name);
                msg.writer().writeUTF(NNTSqlFetcher.loadById(top.id).playerTask.taskMain.name);
                msg.writer().writeUTF("...");
            }
            player.sendMessage(msg);
            msg.cleanup();
    ...
    public static String getTopNap() {
        ...
        ResultSet rs;
        try {
            Connection conn = NNTDB.getConnectionServer();
    ...
    public static String getTopHongNgoc() {
        ...
        try {
            Connection conn = NNTDB.getConnectionServer();
            ps = conn.prepareStatement(ConstSQL.TOP_HONGNGOC);
            conn.setAutoCommit(false);

            rs = ps.executeQuery();
            byte i = 1;
            while (rs.next()) {
                sb.append(i).append(".").append(rs.getString("name")).append(": ").append(rs.getString("total_hongngoc")).append(" Hồng Ngọc\b");
                i++;
            }
            conn.close();
        } catch (Exception e) {
        ...
        try {
            ...
            for (int i = 0; i < tops.size(); i++) {
                ...
                switch (select) {
                    ...
                    case 6 -> {
                        msg.writer().writeUTF("" + Util.numberToMoney(top.getHongngoc()) + " Hồng Ngọc");
//                        msg.writer().writeUTF("" + top.getCash() + " VNĐ");
                    }
//                    case 6 -> {
//                        msg.writer().writeUTF("" + Util.numberToMoney(top.getCash()) + " VNĐ");
