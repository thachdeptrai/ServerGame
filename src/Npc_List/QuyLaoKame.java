...
//                menu.add("Bảng\n Xếp hạng\nNhiệm vụ");
            ...
            if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                this.createOtherMenu(player, ConstNpc.BASE_MENU, "|7|Con Đang Có: " + player.getSession().cash + " VNĐ\n|0|"
                        + "Con muốn hỏi gì nào?", menus);
            }
        }
    }
...
//                            ShopService.gI().opendShop(player, "SKILL_NEW", true);
                        ...
                        case 4 -> {
                            Input.gI().NAPTIENBANK(player);
                            break;
                        }
                        case 5 -> {
                            if (player.getSession().actived) {
                                TopService.openTransactionUI(player);
                            } else {
...
//                                player.inventory.itemsMailBox.addAll(GodGK.getMailBox(player));
                ...
                case ConstNpc.CONFIRM_TELE_NAMEC -> {
                    switch (select) {
                        case 0 -> { // Người chơi chọn "Quét QR"
                            String qrLink = player.getQrLink(); // Lấy link QR đã lưu trước đó
                            NpcService.gI().createBigMessage(player, avartar,
                                    "Khi ấn quét sẽ tự nhảy qua trình duyệt web \n"
                                    + "hãy quét qr ở đó nếu có gì thắc mắc ib \n"
                                    + "|3|zalo: 0867487105 hoặc key vàng \n"
                                    + "để được giải quyết",
                                    (byte) 1, "Quét QR", qrLink);
                        }
                        case 1 -> { // Người chơi chọn "Từ chối"
//                            Service.gI().sendThongBao(player, "Bạn đã từ chối giao dịch.");
                        }
                        default -> {
                            break;
                        }
