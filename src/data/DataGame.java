 ...
 * @author NgocThach
 */
import static data.ImageUtil.encryptImage;
import static data.ImageUtil.encryptString;
import static data.ImageUtil.generateRandomKey;
...
public class DataGame {
    ...
    public static byte vsSkill = 14;
    public static byte vsItem = 14;
    public static int vsRes = 14;
    public static int maxSmallVersion = 42000;
    ...
    public static void sendDataImageVersion(MySession session) {
        ...
        try {
            msg = new Message(-111);
            msg.writer().writeShort(0);
            msg.writer().writeUTF("MeoMeo");
            msg.writer().writeByte(0);
            msg.writer().writeUTF("Ngoc Thach");
...
//    }
    ...
    public static void sendRes(MySession session) {
        try {
            File folder = new File("data/res/x" + session.zoomLevel);
            File[] files = folder.listFiles();

            if (files == null || files.length == 0) {
                System.out.println("⚠️ Không tìm thấy file để gửi trong: " + folder.getAbsolutePath());
                return;
            }
            ...
            for (File fileEntry : files) {
                ...
                if (res == null || res.length == 0) {
                    System.out.println("⚠️ File trống hoặc không đọc được: " + original);
                    continue;
                }
