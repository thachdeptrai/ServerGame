 ...
 * @author NgocThach
 */
import Boss_Manager.OtherBossManager;
import Boss_Manager.LunarNewYearEventManager;
import Boss_Manager.RedRibbonHQManager;
import Boss_Manager.HungVuongEventManager;
import Boss_Manager.ChristmasEventManager;
import Boss_Manager.BrolyManager;
import Boss_Manager.HalloweenEventManager;
import Boss_Manager.TrungThuEventManager;
import Boss_Manager.TreasureUnderSeaManager;
import Boss_Manager.SkillSummonedManager;
import Boss_Manager.FinalBossManager;
import Boss_Manager.AnTromManager;
import Boss_Manager.SnakeWayManager;
import Boss_Manager.GasDestroyManager;
import Boss_Manager.YardartManager;
import BotTelegram.MyTelegramBot;
import consts.ConstPlayer;
import Interface.IBossOutfit;
...
public class Boss extends Player implements IBoss, IBossOutfit {
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            ...
            System.out.println("=> Boss " + this.name + " vừa bị tiêu diệt bởi " + plKill.name);
        }
    ...
    public void wakeupAnotherBossWhenAppear() {
        if (!MapService.gI().isMapMaBu(this.zone.map.mapId) && !MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
            if (this.bossType != BossType.YARDART && this.bossType != BossType.FINAL) {
                String b = ("BOSS " + this.name + " : " + this.zone.map.mapName + " khu vực " + this.zone.zoneId + "(" + this.zone.map.mapId + ")\n");
                NgocRongOnline.gI().myTelegramBot = new MyTelegramBot();
                String chatId = NgocRongOnline.gI().myTelegramBot.chatid;
//                NgocRongOnline.gI().myTelegramBot.sendLogMessage(chatId, b);
                System.out.print(b);
            }
        }
        if (this.bossAppearTogether == null || this.bossAppearTogether[this.currentLevel] == null) {
    ...
    protected void goToXY(int x, int y, boolean isTeleport) {
        if (!isTeleport) {
            byte dir = (byte) (this.location.x - x < 0 ? 1 : -1);/*telegram @tomihjhj_bot*/
            byte move = (byte) Util.nextInt(50, 100);/*telegram @tomihjhj_bot*/
            PlayerService.gI().playerMove(this, this.location.x + (dir == 1 ? move : -move), y);/*telegram @tomihjhj_bot*/
        } else {
            ChangeMapService.gI().changeMapYardrat(this, this.zone, x, y);/*telegram @tomihjhj_bot*/
        }
    }

    protected void goToPlayer(Player pl, boolean isTeleport) {
        goToXY(pl.location.x, pl.location.y, isTeleport);/*telegram @tomihjhj_bot*/
    }
}
