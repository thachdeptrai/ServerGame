...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            NNTDB.executeUpdate("insert into player"
            ...
            String a = ("Tạo player mới thành công!\n");
            Logger.success(a);
//            NgocRongOnline.gI().myTelegramBot = new MyTelegramBot();
//            String chatID = NgocRongOnline.gI().myTelegramBot.chatid;
//            NgocRongOnline.gI().myTelegramBot.sendLogMessage(chatID, a);
            return true;
        } catch (Exception e) {
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                dataArray.add((player.itemTime.isUseHOIXUANDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeHOIXUANDAN)) : 0));
                dataArray.add((player.itemTime.isUseCUUCHUYENKIMDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeCUUCHUYENKIMDAN)) : 0));
                dataArray.add((player.itemTime.isUseNGUNGTHANDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeNGUNGTHANDAN)) : 0));
                dataArray.add((player.itemTime.isUseTHANTAMDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeTHANTAMDAN)) : 0));
                dataArray.add((player.itemTime.isUseLINHHUDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLINHHUDAN)) : 0));
                dataArray.add((player.itemTime.isUseTRUTIENDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeTRUTIENDAN)) : 0));
                dataArray.add((player.itemTime.isUseHOALONGDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeHOALONGDAN)) : 0));
                dataArray.add((player.itemTime.isUseTHIENMENHDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeTHIENMENHDAN)) : 0));
                dataArray.add((player.itemTime.isUseDIEUAMDAN ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeDIEUAMDAN)) : 0));
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "hp_point_fusion = ?, mp_point_fusion = ?, dame_point_fusion = ?, diemdanh = ? ,data_diem = ?  where id = ?";

                NNTDB.executeUpdate(query,
                        player.head,
                ...
                if (player.isOffline) {
                    ...
                    String c = (TimeUtil.getCurrHour() + "h" + TimeUtil.getCurrMin() + "m: Player " + player.name + " saved successfully! " + (System.currentTimeMillis() - st) + "ms\n");
//                    Logger.success(c);
                }
            } catch (Exception e) {
    ...
    public static boolean addHongNgoc(Player player, int amount) {
        String updateQuery = "UPDATE account SET hongngoc = hongngoc + ? WHERE id = ?";
        try (Connection con = NNTDB.getConnectionServer(); PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ps.setInt(1, amount);
            ps.setInt(2, player.getSession().userId);
            int rowsAffected = ps.executeUpdate();
            ...
            Logger.logException(PlayerDAO.class, e, "Error updating hongngoc for player " + player.name);
            return false;
        }
    }

    public static boolean subHongNgoc(Player player, int num) {
        String updateQuery = "UPDATE account SET hongngoc = hongngoc - ? WHERE id = ?";
        ...
        try (Connection con = NNTDB.getConnectionServer(); PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ...
            int rowsAffected = ps.executeUpdate();
            if (rowsAffected > 0) {
                player.getSession().hongngoc -= num;
                return true;
            } else {
                return false;
            }
    ...
    public static void updateTotalHongNgoc(Player player, int amount) {
        String updateQuery = "UPDATE account SET total_hongngoc = total_hongngoc + ? WHERE id = ?";
        ...
        try (Connection con = NNTDB.getConnectionServer(); PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ...
            Logger.logException(PlayerDAO.class, e, "Error updating total_hongngoc for player " + player.name);
        }
    }
    ...
    public static void updateLastTimeUpdatePower(Player player, long num) {
        ...
        try (Connection con = NNTDB.getConnectionServer(); PreparedStatement ps = con.prepareStatement(query)) {
            ps.setLong(1, num);
...
//    public static void getHongNgoc() {
//        try (Connection conn = NNTDB.getConnectionServer()) {
//            PreparedStatement preparedStatement = conn.prepareStatement("SELECT hongngoc FROM account WHERE id = ?");
//            ResultSet rs = preparedStatement.executeQuery();
//            if (rs.next()) {
//                int hongNgoc = rs.getInt("hongngoc"); // Lấy giá trị từ cột 'hongngoc'
////                System.out.println("Số lượng hồng ngọc: " + hongNgoc);
//            } else {
