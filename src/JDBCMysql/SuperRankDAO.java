...
public class SuperRankDAO {
    ...
    public static int getHighestRank() {
        ...
        try {
            rs = NNTDB.executeQuery("SELECT rank FROM player ORDER BY rank DESC LIMIT 1");
            if (rs.next()) {
    ...
    public static List<Long> getPlayerListInRankRange(int rank, int limit) {
        ...
        if (rand != -1) {
            try {
                rs = NNTDB.executeQuery("SELECT id FROM player WHERE rank = ? LIMIT 1", rand);
                if (rs.next()) {
                    list.add((long) rs.getInt("id"));
                }
            } catch (Exception e) {
    ...
    public static List<Long> getPlayerListInRank(int rank, int limit) {
        ...
        try {
            rs = NNTDB.executeQuery("SELECT id FROM player WHERE rank > 0 ORDER BY rank ASC LIMIT ?", limit);
            while (rs.next()) {
                list.add((long) rs.getInt("id"));
            }
        } catch (Exception e) {
    ...
    public static void updateRank(Player player) {
        try {
            ...
            NNTDB.executeUpdate(query, player.superRank.rank, player.id);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark.isLoadedAllDataPlayer()) {
            try {
                ...
                NNTDB.executeUpdate(query, inventory, player.superRank.rank, dataSuperRank, player.id);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
