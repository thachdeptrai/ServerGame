...
import data.BadgesData;
import player.DailyGiftData;
...
public class NNTSqlFetcher {
    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        Player plInGame;
        try {
            rs = NNTDB.executeQuery("select * from account where username = ? and password = ?", session.uu, session.pp);
            if (rs.next()) {
                ...
                session.hongngoc = rs.getInt("hongngoc");
...
//                } else
                if (rs.getBoolean("ban")) {
                    ...
                    if (secondsPass < Manager.SECOND_WAIT_LOGIN) {
                        ...
                        rs = NNTDB.executeQuery("select * from player where account_id = ? limit 1", session.userId);
                        if (!rs.next()) {
                            ...
                            if ((player = loadPlayer(rs, false)) != null) {
                                ...
                                NNTDB.executeUpdate("update account set last_time_login = '" + new Timestamp(System.currentTimeMillis()) + "', ip_address = '" + session.ipAddress + "' where id = " + session.userId);
                            }
                        }
                    }
                }
            ...
            Logger.logException(NNTSqlFetcher.class, e);
        } finally {
            if (rs != null) {
                rs.dispose();
            }
    ...
    public static Player loadById(long id) {
        ...
        try {
            ...
            rs = NNTDB.executeQuery("select * from player where id = ? limit 1", id);
    ...
    private static Player loadPlayer(NTTResultSet rs, boolean isOffline) throws Exception {
        Player player = null;
        try {
            double plHp;
            double plMp;
...
//            }
            ...
            if (dataArray.size() > 29) {
                timeUseHXD = Integer.parseInt(String.valueOf(dataArray.get(29)));
            }
            if (dataArray.size() > 30) {
                timeUseCCKD = Integer.parseInt(String.valueOf(dataArray.get(30)));
            }
            if (dataArray.size() > 31) {
                timeUseNTD = Integer.parseInt(String.valueOf(dataArray.get(31)));
            }
            if (dataArray.size() > 32) {
                timeUseTTD = Integer.parseInt(String.valueOf(dataArray.get(32)));
            }
            if (dataArray.size() > 33) {
                timeUseLTD = Integer.parseInt(String.valueOf(dataArray.get(33)));
            }
            if (dataArray.size() > 34) {
                timeUseTTD2 = Integer.parseInt(String.valueOf(dataArray.get(34)));
            }
            if (dataArray.size() > 35) {
                timeUseHLD = Integer.parseInt(String.valueOf(dataArray.get(35)));
            }
            if (dataArray.size() > 36) {
                timeUseTMD = Integer.parseInt(String.valueOf(dataArray.get(36)));
            }
            if (dataArray.size() > 37) {
                timeUseDAD = Integer.parseInt(String.valueOf(dataArray.get(37)));
            }
            ...
            player.itemTime.lastTimeHOIXUANDAN = System.currentTimeMillis() - (ItemTime.TIME_CMS - timeUseHXD);
            player.itemTime.lastTimeCUUCHUYENKIMDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseCCKD);
            player.itemTime.lastTimeNGUNGTHANDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseNTD);
            player.itemTime.lastTimeTHANTAMDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseTTD);
            player.itemTime.lastTimeLINHHUDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseLTD);
            player.itemTime.lastTimeTRUTIENDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseTTD2);
            player.itemTime.lastTimeHOALONGDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseHLD);
            player.itemTime.lastTimeTHIENMENHDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseTMD);
            player.itemTime.lastTimeDIEUAMDAN = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseDAD);
            ...
            player.itemTime.isUseHOIXUANDAN = timeUseHXD != 0;
            player.itemTime.isUseCUUCHUYENKIMDAN = timeUseCCKD != 0;
            player.itemTime.isUseNGUNGTHANDAN = timeUseNTD != 0;
            player.itemTime.isUseTHANTAMDAN = timeUseTTD != 0;
            player.itemTime.isUseLINHHUDAN = timeUseLTD != 0;
            player.itemTime.isUseTRUTIENDAN = timeUseTTD2 != 0;
            player.itemTime.isUseHOALONGDAN = timeUseHLD != 0;
            player.itemTime.isUseTHIENMENHDAN = timeUseTMD != 0;
            player.itemTime.isUseDIEUAMDAN = timeUseDAD != 0;
            dataArray.clear();
...
//            }
    ...
    public static List<Player> getAllPlayer() {
        try {
            ...
            try {
                ...
                rs = NNTDB.executeQuery("select * from player");
                while (rs.next()) {
                    double plHp = Double.MAX_VALUE;
                    double plMp = Double.MAX_VALUE;
    ...
    public static Player loadPlayerByName(String name) {
        ...
        try {
            ...
            rs = NNTDB.executeQuery("select * from player where name = ? limit 1", name);
            if (rs.first()) {
                double plHp = Double.MAX_VALUE;
                double plMp = Double.MAX_VALUE;
                JSONValue jv = new JSONValue();
...
//    }
    public static boolean updateMailBox(Player player) {
        try {
            ...
            dataArray.clear();
            try (Connection con = NNTDB.getConnectionServer(); PreparedStatement ps = con.prepareStatement("update `player` set item_mails_box = ? where id = ?")) {
